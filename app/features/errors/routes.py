"""
app/features/errors/routes.py

This is how i will make the things different errors
messages will handle by this routes
When i will do abort(400) like this the below thigns will run
"""

from werkzeug.exceptions import HTTPException, BadRequest

from flask import Blueprint, render_template

error_bp = Blueprint(
    name="error_bp",
    import_name=__name__,
    template_folder="templates",
)


# This is for 404 when user visit the wrong /endpiont it will run
@error_bp.app_errorhandler(404)
def handle_404(error: HTTPException):
    return render_template(
        template_name_or_list="error/404.html",
    )


# Here the error.description has a default value
# from the flask it will shows if i will not shows
@error_bp.app_errorhandler(400)
def handle_bad_request(error: HTTPException):

    if error.description != BadRequest.description:
        show_msg = error.description
    else:
        show_msg = None

    return (
        render_template(
            template_name_or_list="error/400.html",
            show_msg=show_msg,
        ),
        400,
    )


@error_bp.app_errorhandler(401)
def handle_401(error: HTTPException):

    print("Thsi 401 running")
    if error.description != BadRequest.description:
        show_msg = error.description
    else:
        show_msg = None
    return (
        render_template(
            template_name_or_list="error/401.html",
            shws_msg=show_msg,
        ),
        401,
    )
